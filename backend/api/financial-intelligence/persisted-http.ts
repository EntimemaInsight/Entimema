...
import {customerFinancialRun} from "./http";
...
export const createRunHandlers=(deps:Deps)=>({GET:async(_request:Request,context:{params:Promise<{runId:string}>})=>{const id=randomUUID();try{const actor=await deps.authorize(),{runId}=await context.params;const run=await deps.service.get(actor.actorId,runId);return run?json(customerFinancialRun(run)):json({error_code:"RUN_NOT_FOUND"},404)}catch(e){return fail(e,id)}}});
export const createResolverTelemetryHandler=(deps:Deps)=>async(_request:Request,context:{params:Promise<{runId:string}>})=>{const id=randomUUID();try{const actor=await deps.authorize(),{runId}=await context.params;const run=await deps.service.get(actor.actorId,runId);return run?json(run.resolverTelemetry):json({error_code:"RUN_NOT_FOUND"},404)}catch(e){return fail(e,id)}};
const identity=(request:Request)=>{const q=new URL(request.url).searchParams,revision=Number(q.get("revision")),selectedStatement=q.get("statement")??"",snapshotHash=q.get("snapshot")??"",schemaVersion=q.get("schema")??"";if(!Number.isInteger(revision)||!selectedStatement||!snapshotHash||!schemaVersion)throw new AgentError("FILE_CORRUPT",400);return{revision,selectedStatement,snapshotHash,schemaVersion}};
export const createAnalysisHandler=(deps:Deps)=>async(request:Request,context:{params:Promise<{runId:string}>})=>{const id=randomUUID();try{const actor=await deps.authorize(),{runId}=await context.params;return json(await deps.service.analyze(actor.actorId,runId,identity(request)))}catch(e){if(e instanceof Error&&["ANALYSIS_REQUIRES_VALIDATED_RUN","STALE_ANALYSIS_IDENTITY"].includes(e.message))return json({error_code:"ANALYSIS_BLOCKED",message:"Financial analysis requires the current validated snapshot."},409);return fail(e,id)}};
