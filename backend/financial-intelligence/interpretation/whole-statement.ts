...
const resolverOutcome=(reason:string|null,resolution:boolean):ResolverOutcome=>resolution?"success":reason==="feature_disabled"?"disabled":reason==="timeout"?"timeout":reason==="invalid_output"?"invalid_schema":["provider_error","provider_unavailable","rate_limit","invalid_request"].includes(reason??"")?"provider_error":"unresolved";
