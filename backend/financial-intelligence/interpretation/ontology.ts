...
const insignificantTokens=new Set(["and","of","the","total","net","cost","costs","expense","expenses"]);
const conceptTokens=(concept:CanonicalConcept)=>normalizeAccountingLabel(concept.replaceAll("_"," ")).split(" ").filter(token=>!insignificantTokens.has(token));
/** Partial similarity ranks the bounded ontology but can never auto-select a concept. */
const partialLexicalScore=(label:string,concept:CanonicalConcept)=>{const source=new Set(label.split(" ").filter(token=>!insignificantTokens.has(token))),target=conceptTokens(concept);if(!target.length||!target.every(token=>source.has(token)))return 0;return target.length/source.size};
