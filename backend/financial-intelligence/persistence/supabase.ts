...
export class SupabaseFinancialRunRepository implements FinancialRunRepository{
 ...
 async list(ownerId:string){const rows=await call(`financial_runs?owner_id=eq.${encodeURIComponent(ownerId)}&select=run_id,status,revision,created_at,updated_at,contract&order=updated_at.desc`);return this.listRows(rows)}
 async get(ownerId:string,runId:string){const rows=await call(`financial_runs?owner_id=eq.${encodeURIComponent(ownerId)}&run_id=eq.${encodeURIComponent(runId)}&select=*,audit_events:financial_audit_events(event_id,event_type,actor_id,created_at,revision)&limit=1`);if(!rows.length)return null;rows[0].audit_events=(rows[0].audit_events??[]).map((e:any)=>({eventId:e.event_id,eventType:e.event_type,actor:e.actor_id==="system"?"System":"Workspace user",timestamp:e.created_at,revision:e.revision})).sort((a:any,b:any)=>a.timestamp.localeCompare(b.timestamp));return hydrate(rows[0])}
