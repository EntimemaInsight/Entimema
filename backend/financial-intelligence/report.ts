...
export function renderFinancialReportPdf(report: HashedFinancialReport): Buffer {
  ...
  pdf.addPage("Technical appendix");pdf.kicker("Evidence appendix");pdf.heading("Evidence and integrity","Compact source coordinates for every analytical input. Full workbook bytes are not retained.");
  const evidenceTop=pdf.page.y-18, evidenceBottom=225, rowHeight=10, columnWidth=252, rowsPerColumn=Math.floor((evidenceTop-evidenceBottom)/rowHeight);
  [MARGIN,MARGIN+columnWidth].forEach((x)=>{pdf.at("REF",x,evidenceTop+13,4.6,"bold","muted",.6);pdf.at("SOURCE CELL",x+45,evidenceTop+13,4.6,"bold","muted",.6);pdf.at("LINE",x+105,evidenceTop+13,4.6,"bold","muted",.6);pdf.at("PERIOD",x+188,evidenceTop+13,4.6,"bold","muted",.6);pdf.at("VALUE",x+246,evidenceTop+13,4.6,"bold","muted",.6,"right");});
  payload.evidence.forEach((evidence,index)=>{const pageIndex=Math.floor(index/(rowsPerColumn*2));const localIndex=index%(rowsPerColumn*2);if(pageIndex>0&&localIndex===0){pdf.addPage("Technical appendix");pdf.kicker("Evidence appendix");pdf.heading("Evidence continued","Additional source coordinates.");}const column=Math.floor(localIndex/rowsPerColumn),row=localIndex%rowsPerColumn,x=MARGIN+column*columnWidth,y=evidenceTop-row*rowHeight;const ref=evidence.id.slice(-8),source=(evidence.sheetName??`p.${evidence.pageNumber??"-"}`).slice(0,14),cell=evidence.cellAddress??"-",line=String(evidence.rawRowLabel??"").slice(0,17),period=String(evidence.rawColumnHeader??"").slice(0,8),raw=String(evidence.rawCellValue??"-").slice(0,9);pdf.at(ref,x,y,4.8,"sans","accent");pdf.at(`${source}!${cell}`,x+45,y,4.8,"bold","ink");pdf.at(line,x+105,y,4.7,"sans","ink");pdf.at(period,x+188,y,4.6,"sans","muted");pdf.at(raw,x+246,y,4.7,"sans","ink",0,"right");});
  pdf.page.y=190;pdf.line(MARGIN,pdf.page.y+12,PAGE_WIDTH-MARGIN,pdf.page.y+12,"0.55 0.09 0.13",.7);pdf.at("INTEGRITY",MARGIN,pdf.page.y,6.5,"bold","accent",1.2);pdf.page.y-=18;[["Run",payload.runId],["Revision",String(payload.revision)],["Versions",`${payload.analysisVersion} / ${payload.schemaVersion}`],["Generated",payload.generatedAt],["Validated",payload.validatedAt]].forEach(([key,value],index)=>{const col=index%2,row=Math.floor(index/2),x=MARGIN+col*258,y=pdf.page.y-row*24;pdf.at(`${key}:`,x,y,5.4,"bold","muted");pdf.at(String(value),x+58,y,5.2,"sans","ink");});
  pdf.at("Snapshot:",MARGIN,96,5.2,"bold","muted");pdf.at(payload.validatedSnapshotIntegrityReference,MARGIN+58,96,4.8,"sans","ink");
  pdf.at("Payload SHA-256:",MARGIN,78,5.2,"bold","muted");pdf.at(payloadHash,MARGIN+80,78,4.8,"sans","accent");
