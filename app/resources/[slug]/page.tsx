...
export default async function ResourcePage({ params }: PageProps<"/resources/[slug]">) {
  ...
  if (isCreditPolicyRules || isAffordabilityDecisioning || isCreditLimitAssignment || isRiskBasedPricing || isChampionChallengerStrategy || isDecisionEngineMonitoring || isConsumerCreditEarlyWarning || isBehaviouralCreditScoring || isCollectionsPrioritisation || isCureRedefaultAnalytics || isPromiseToPayAnalytics || isHiddenInfrastructureDebt || isPaymentIsNotBalance || isSingleCustomerView) {
    return (
      <>
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: serializeJsonLd(articleSchema) }} />
        <ResourceArticle resource={resource} sections={isSingleCustomerView ? [...singleCustomerViewSections] : isPaymentIsNotBalance ? [...paymentIsNotBalanceSections] : isHiddenInfrastructureDebt ? [...hiddenInfrastructureDebtSections] : isPromiseToPayAnalytics ? [...promiseToPayAnalyticsSections] : isCureRedefaultAnalytics ? [...cureRedefaultAnalyticsSections] : isCollectionsPrioritisation ? [...collectionsPrioritisationSections] : isBehaviouralCreditScoring ? [...behaviouralCreditScoringSections] : isConsumerCreditEarlyWarning ? [...consumerCreditEarlyWarningSections] : isDecisionEngineMonitoring ? [...decisionEngineMonitoringSections] : isChampionChallengerStrategy ? [...championChallengerStrategySections] : isRiskBasedPricing ? [...riskBasedPricingSections] : isCreditLimitAssignment ? [...creditLimitAssignmentSections] : isAffordabilityDecisioning ? [...affordabilityDecisioningSections] : [...creditPolicyRulesSections]}>
          {isSingleCustomerView ? <SingleCustomerViewArticle /> : isPaymentIsNotBalance ? <PaymentIsNotBalanceArticle /> : isHiddenInfrastructureDebt ? <HiddenInfrastructureDebtArticle /> : isPromiseToPayAnalytics ? <PromiseToPayAnalyticsArticle /> : isCureRedefaultAnalytics ? <CureRedefaultAnalyticsArticle /> : isCollectionsPrioritisation ? <CollectionsPrioritisationArticle /> : isBehaviouralCreditScoring ? <BehaviouralCreditScoringArticle /> : isConsumerCreditEarlyWarning ? <ConsumerCreditEarlyWarningArticle /> : isDecisionEngineMonitoring ? <DecisionEngineMonitoringArticle /> : isChampionChallengerStrategy ? <ChampionChallengerStrategyArticle /> : isRiskBasedPricing ? <RiskBasedPricingArticle /> : isCreditLimitAssignment ? <CreditLimitAssignmentArticle /> : isAffordabilityDecisioning ? <AffordabilityDecisioningArticle /> : <CreditPolicyRulesArticle />}
        </ResourceArticle>
      </>
    );
  }
