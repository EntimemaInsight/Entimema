...
export const creditRiskCutOffSections = [
  { id: "probability-not-decision", label: "Probability is not a decision" },
  { id: "economics", label: "The economics of approval" },
  { id: "decision-zones", label: "From one cut-off to decision zones" },
  { id: "portfolio-frontier", label: "The portfolio strategy frontier" },
  { id: "strategy-system", label: "Pricing, limits, policy and appetite" },
  { id: "robustness", label: "Robustness, uncertainty and optimisation" },
  { id: "decision-engine", label: "From cut-off to decision engine" },
  { id: "monitoring", label: "Monitor the strategy as a system" },
] as const;
...
export default function CreditRiskCutOffArticle() {
  return <>
    <p className="resource-lead"><em>A cut-off is not a statistical property of a credit model. It is an economic decision boundary imposed on the model&apos;s risk signal.</em></p>

    <section id="probability-not-decision">
      <h2>Probability is not a decision</h2>
    ...
    <section id="portfolio-frontier">
      ...
      <ResourceFigure label="Strategy frontier contrasting strict and loose credit cut-offs with expected value as the optimisation layer." caption="Expected value mediates the volume–loss trade-off; constraints determine which economically attractive point is admissible.">
        <div className={styles.frontier}><article><small>STRICTER CUT-OFF</small><strong>Lower approval</strong><span>Lower expected loss</span></article><div><small>OPTIMISATION LAYER</small><strong>EXPECTED VALUE</strong><span>Revenue − loss − cost</span></div><article><small>LOOSER CUT-OFF</small><strong>Higher approval</strong><span>Higher expected loss</span></article></div>
      </ResourceFigure>
    </section>
    ...
    <section id="monitoring">
      ...
      <h3>Failure modes that make cut-offs look precise while weakening decisions</h3>
      <ul>
        <li>choosing the cut-off from KS, ROC or classification accuracy alone;</li>
        <li>maximising approval without loss economics—or minimising bad rate regardless of revenue;</li>
        <li>using an uncalibrated score as though it were an economically meaningful PD;</li>
        <li>applying one universal threshold across materially different products and economics;</li>
        <li>ignoring LGD, exposure, utilisation, capital and operating cost;</li>
        <li>treating manual review as free and infinitely scalable;</li>
        <li>allowing policy rules and overrides to operate outside monitored decision logic;</li>
        <li>optimising on development data without sensitivity, uncertainty or vintage evidence;</li>
        <li>changing thresholds to meet short-term volume targets without tracing later loss;</li>
        <li>monitoring the model while leaving the downstream strategy unmonitored.</li>
      </ul>
