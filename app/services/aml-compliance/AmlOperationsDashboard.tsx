...
export default function AmlOperationsDashboard() {
  return (
    <div className={styles.dashboard} aria-label="AML Operations dashboard">
      <div className={styles.dashboardTop}><div><span className={styles.dashboardEyebrow}>Enterprise AML platform</span><h2>AML Operations</h2></div><span className={styles.demoBadge}>Live controls</span></div>
      ...
      <div className={styles.riskGrid}>
        <section className={styles.chartPanel} aria-labelledby="alert-trend-title"><div className={styles.chartHeading}><div><span>Monitoring</span><h3 id="alert-trend-title">Alert Trend</h3></div><span className={styles.miniLegend}>30 days</span></div><svg className={styles.lineChart} viewBox="0 0 420 135" role="img" aria-label="Тенденция на AML сигналите"><g className={styles.gridLines}><path d="M10 25H410M10 67H410M10 109H410" /></g><path className={styles.actualLine} d="M12 96C48 88 76 104 112 77S177 83 214 56S283 65 320 42S373 48 408 25"/><path className={styles.forecastLine} d="M12 111C55 105 89 99 125 94S190 82 230 78S302 66 342 62S383 55 408 50"/></svg></section>
        ...
        <section className={styles.funnelPanel} aria-labelledby="cases-status-title"><div className={styles.chartHeading}><div><span>Investigations</span><h3 id="cases-status-title">Cases by Status</h3></div></div><div className={styles.funnel}>{[["New", "100%"], ["Review", "74%"], ["Escalated", "39%"], ["Filed", "18%"]].map(([label, width]) => <div key={label}><span>{label}</span><i style={{ "--width": width } as CSSProperties} /><b>{width}</b></div>)}</div></section>
      ...
      <div className={styles.dashboardFooter} aria-label="Suspicious Activity Timeline"><span>Alert detected</span><span>Analyst review</span><span>MLRO escalation</span><span>Regulatory filing</span></div>
    </div>
  );
}
