...
const metrics = [
  ["Connected Sources", "12 / 12", "All synchronized"],
  ["Data Quality", "99.4%", "+0.8 pts"],
  ["Automation Rate", "94%", "18 active flows"],
...
export default function FinancialDataDashboard() {
  return (
    <div className={styles.dashboard} aria-label="Демонстрационен контролен панел за надеждност и интеграция на финансови данни">
      <div className={styles.dashboardTop}><div><span className={styles.dashboardEyebrow}>Financial data foundation</span><h2>Data Reliability Control</h2></div><span className={styles.demoBadge}>Synchronized</span></div>
      ...
      <div className={styles.charts}>
        <section className={styles.chartPanel} aria-labelledby="data-flow-title"><div className={styles.chartHeading}><div><span>Data Flow</span><h3 id="data-flow-title">Integrated Financial Flow</h3></div></div><div className={styles.dataFlow}><span>ERP</span><i /><span>Model</span><i /><span>Finance</span></div></section>
        <section className={styles.cashPanel} aria-labelledby="source-mapping-title"><div className={styles.chartHeading}><div><span>Source Mapping</span><h3 id="source-mapping-title">Mapped Structures</h3></div></div><div className={styles.mappingList}>{[["Chart of Accounts", "100%"], ["Cost Centers", "98%"], ["Legal Entities", "100%"]].map(([label, width]) => <div className={styles.mappingRow} key={label}><span>{label}</span><span className={styles.mappingTrack}><i style={{ "--width": width } as CSSProperties} /></span><strong>{width}</strong></div>)}</div></section>
        ...
        <section className={styles.cashPanel} aria-labelledby="integration-status-title"><div className={styles.chartHeading}><div><span>Integration Status</span><h3 id="integration-status-title">Connected Systems</h3></div></div><div className={styles.systemList}>{["ERP Core", "Planning", "Reporting", "Master Data"].map((system) => <div key={system}><i /><span>{system}</span><strong>Live</strong></div>)}</div></section>
      </div>
    </div>
  );
}
