  ...
  return (
    <div className={styles.dashboard} aria-label="Illustrative credit risk management dashboard">
      ...
      <div className={styles.riskGrid}>
        ...
        <section className={styles.matrixPanel} aria-labelledby="migration-title"><div className={styles.chartHeading}><div><span>TRANSITIONS</span><h3 id="migration-title">DPD Transition Matrix</h3></div></div><div className={styles.matrix}>{[92, 6, 2, 1, 81, 15, 0, 12, 72].map((value, index) => <i key={index} style={{ "--alpha": `${Math.max(value, 8) / 100}` } as CSSProperties}>{value}%</i>)}</div></section>
        <section className={styles.chartPanel} aria-labelledby="vintage-title"><div className={styles.chartHeading}><div><span>PORTFOLIO MONITORING</span><h3 id="vintage-title">Vintage Curves</h3></div><span className={styles.miniLegend}>MOB 1–6</span></div><svg className={styles.lineChart} viewBox="0 0 420 135" role="img" aria-label="Comparison of three vintage curves"><g className={styles.gridLines}><path d="M10 25H410M10 67H410M10 109H410" /></g><path className={styles.actualLine} d="M12 111C72 102 104 90 151 78S241 55 288 45S355 31 408 26"/><path className={styles.forecastLine} d="M12 116C70 111 108 101 151 91S239 72 288 63S357 53 408 47"/><path className={styles.thirdLine} d="M12 119C72 116 109 111 151 105S240 94 288 87S360 77 408 72"/></svg></section>
        <section className={`${styles.strategyPanel} ${styles.policyPanel}`} aria-labelledby="strategy-title"><div className={styles.chartHeading}><div><span>MODEL + POLICY</span><h3 id="strategy-title">Champion vs Challenger</h3></div></div><div className={styles.strategyScore}><strong>+8.4%</strong><span>risk-adjusted approval</span></div><div className={styles.scoreBands}>{scoreBands.map(([label, width]) => <div key={label}><span>{label}</span><i><b style={{ "--width": width } as CSSProperties}/></i><small>{width}</small></div>)}</div></section>
      </div>
      <div className={styles.dashboardFooter}><span>Score Distribution</span><span>PD Distribution</span><span>Roll Rates</span><span>Portfolio Segmentation</span></div>
    </div>
  );
}
