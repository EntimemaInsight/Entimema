...
test("regression: compatible 0.90 semantic evidence reaches provisional and final projection without double discounting",()=>withResolver(async()=>{
 ...
 const submitted=(request!.rows as Array<{rowNumber:number;candidateSetId:number}>).find(row=>row.rowNumber===2)!;const candidateSets=request!.candidateSets as string[][];assert.deepEqual(new Set(candidateSets[submitted.candidateSetId]),new Set(P_AND_L_CONCEPTS));
}));
...
test("generic composite operating-expense language prioritizes an existing candidate and remains model-owned",()=>withResolver(async()=>{
 ...
 const submittedRow=(submitted!.rows as Array<{rowNumber:number;candidateSetId:number}>).find(row=>row.rowNumber===2)!;const candidates=(submitted!.candidateSets as string[][])[submittedRow.candidateSetId];
