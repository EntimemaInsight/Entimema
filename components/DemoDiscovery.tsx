...
function SelectField({ error, id, label, name, options }: { error?: string; id: string; label: string; name: string; options: readonly string[] }) {
  const errorId = `${id}-error`;
  return <label className={styles.field} htmlFor={id}><span>{label} *</span><select aria-describedby={error ? errorId : undefined} aria-invalid={Boolean(error)} defaultValue="" id={id} name={name}><option disabled value="">Select</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>{error && <small className={styles.fieldError} id={errorId}>{error}</small>}</label>;
}
